...
# API server is not exposed to the public network
jq --arg ip_address "$CURRENT_NODE_IP_ADDRESS" '.Addresses.API = ["/ip4/127.0.0.1/tcp/5001", "/ip6/::/tcp/5001"] | .Addresses.Gateway = ["/ip4/\($ip_address)/tcp/8080", "/ip6/::/tcp/8080"]' /vol/swarm/$CURRENT_NODE_IP_ADDRESS/ipfs/config > /vol/swarm/$CURRENT_NODE_IP_ADDRESS/ipfs/updated-config
